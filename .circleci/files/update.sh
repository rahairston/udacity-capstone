...
# Run socat to forward port 80 to port 8080
if $(ps -A | grep -q socat)
then
echo "Port 80 is already forwarded to 8080"
else
echo "Forwarding localhost port 80 to the NodePort in Minikube server $(minikube service --url capstone-server)"
sudo nohup socat TCP-LISTEN:80,fork TCP:$(minikube service --url capstone-server | sed -s "s|http://||g") >> /tmp/socat_logs &
fi
